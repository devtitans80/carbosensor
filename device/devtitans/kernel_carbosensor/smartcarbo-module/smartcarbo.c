...
MODULE_DESCRIPTION("Driver de acesso ao SmartCarbo (ESP32 com Chip Serial CP2102");
MODULE_LICENSE("GPL");
...
// Executado quando o arquivo /sys/kernel/smartcarbosensor/{mq7} é lido (e.g., cat /sys/kernel/smartcarbosensor/mq7)
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff);

// Executado quando o arquivo /sys/kernel/smartcarbosensor/{mq7} é escrito (e.g., echo "100" | sudo tee -a /sys/kernel/smartcarbosensor/mq7)
...
static struct usb_device *smartcarbosensor_device;        // Referência para o dispositivo USB
static uint usb_in, usb_out;                       // Endereços das portas de entrada e saida da USB
static char *usb_in_buffer, *usb_out_buffer;       // Buffers de entrada e saída da USB
static int usb_max_size;                           // Tamanho máximo de uma mensagem USB

// Variáveis para criar os arquivos no /sys/kernel/smartcarbosensor/{mq7}
...
// Cria e registra o driver do smartcarbosensor no kernel
static struct usb_driver smartcarbosensor_driver = {
    .name        = "smartcarbosensor",     // Nome do driver
    .probe       = usb_probe,       // Executado quando o dispositivo é conectado na USB
    .disconnect  = usb_disconnect,  // Executado quando o dispositivo é desconectado na USB
    .id_table    = id_table,        // Tabela com o VendorID e ProductID do dispositivo
};
module_usb_driver(smartcarbosensor_driver);

// Executado quando o dispositivo é conectado na USB
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    struct usb_endpoint_descriptor *usb_endpoint_in, *usb_endpoint_out;

    printk(KERN_INFO "SmartCarbo: Dispositivo conectado ...\n");

    // Cria arquivos do /sys/kernel/smartcarbosensor/*
    sys_obj = kobject_create_and_add("smartcarbosensor", kernel_kobj);
    sysfs_create_group(sys_obj, &attr_group);

    // Detecta portas e aloca buffers de entrada e saída de dados na USB
    smartcarbosensor_device = interface_to_usbdev(interface);
...
static void usb_disconnect(struct usb_interface *interface) {
    ...
    if (sys_obj) kobject_put(sys_obj);      // Remove os arquivos em /sys/kernel/smartcarbosensor
...
static int usb_send_cmd(char *cmd, int param) {
    ...
    ret = usb_bulk_msg(smartcarbosensor_device, usb_sndbulkpipe(smartcarbosensor_device, usb_out), usb_out_buffer, strlen(usb_out_buffer), &actual_size, 1000*HZ);
    if (ret) {
        printk(KERN_ERR "SmartCarbo: Erro de codigo %d ao enviar comando!\n", ret);
        return -1;
    }
    ...
    while (retries > 0) {
        // Lê dados da USB
        ret = usb_bulk_msg(smartcarbosensor_device, usb_rcvbulkpipe(smartcarbosensor_device, usb_in), usb_in_buffer, min(usb_max_size, MAX_RECV_LINE), &actual_size, HZ*1000);
        if (ret) {
            printk(KERN_ERR "SmartCarbo: Erro ao ler dados da USB (tentativa %d). Codigo: %d\n", ret, retries--);
            continue;
        }
        ...
        for (i=0; i<actual_size; i++) {
            ...
            if (usb_in_buffer[i] == '\n') {  // Temos uma linha completa
                ...
                printk(KERN_INFO "SmartCarbo: Recebido uma linha: '%s'\n", recv_line);
                ...
                if (!strncmp(recv_line, resp_expected, strlen(resp_expected))) {
                    printk(KERN_INFO "SmartCarbo: Linha eh resposta para %s! Processando ...\n", cmd);
                    ...
                    printk(KERN_INFO "SmartCarbo: Nao eh resposta para %s! Tentiva %d. Proxima linha...\n", cmd, retries--);
...
//     if (ret) {
//         printk(KERN_ALERT "SmartCarbo: valor de %s invalido.\n", attr_name);
//         return -EACCES;
//     }
//
//     printk(KERN_INFO "SmartCarbo: Setando %s para %ld ...\n", attr_name, value);
//
...
//     }
//     if (ret < 0) {
//         printk(KERN_ALERT "SmartCarbo: erro ao setar o valor do %s.\n", attr_name);
//         return -EACCES;
//     }
//
//     return strlen(buff);
